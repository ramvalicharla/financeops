...
def parse_excel_bytes(
    ...
    workbook = load_workbook(
        filename=_bytes_to_stream(content), data_only=False, read_only=True
    )
    visible_sheets = [
    ...
    return {
        ...
        "section_breaks": _find_section_breaks(row_labels),
        "header_row_index": 0,
...
def _find_section_breaks(row_labels: list[str]) -> list[str]:
    labels = [label.strip().lower() for label in row_labels if label.strip()]
    seen: set[str] = set()
    section_breaks: list[str] = []
    for label in labels:
        if label in seen and label not in section_breaks:
            section_breaks.append(label)
        seen.add(label)

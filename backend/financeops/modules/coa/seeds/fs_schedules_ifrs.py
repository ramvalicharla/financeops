...
from financeops.modules.coa.seeds.constants import IFRS_SCHEDULES


async def seed_ifrs_schedules(
    session: AsyncSession,
    *,
    classification_ids: dict[str, str],
) -> dict[str, str]:
    payload = []
    for schedule in IFRS_SCHEDULES:
        payload.append(
            {
    ...
    rows = (
        await session.execute(
            select(CoaFsSchedule.code, CoaFsSchedule.id).where(
                CoaFsSchedule.code.in_([item["code"] for item in IFRS_SCHEDULES])
            )
        )
    ).all()

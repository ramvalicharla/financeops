...
def _sign(payload_segment: str) -> str:
    digest = hmac.new(
        ...
        f"service:{payload_segment}".encode("utf-8"),
        hashlib.sha256,
    ).digest()
    return _b64url_encode(digest)


def issue_service_token(claims: dict[str, Any]) -> str:
    payload = {"auth_mode": "service", **claims}
...
def verify_service_token(token: str) -> dict[str, Any]:
    try:
        ...
        raise AuthenticationError("Invalid service token format") from exc

    expected = _sign(payload_segment)
    if not hmac.compare_digest(signature, expected):
        raise AuthenticationError("Invalid service token signature")

    payload = json.loads(_b64url_decode(payload_segment).decode("utf-8"))
    if payload.get("auth_mode") != "service":
        raise AuthenticationError("Invalid service token type")
    ...
    if not payload.get("service_id"):
        raise AuthenticationError("Service token missing service_id")
    return payload

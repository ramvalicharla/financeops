...
def test_get_weasyprint_mode_returns_string() -> None:
    """Capability probe returns one of supported mode strings."""
...
def test_weasyprint_mode_real_when_render_succeeds(monkeypatch: pytest.MonkeyPatch) -> None:
    """Mode reports real when HTML.write_pdf succeeds."""
...
def test_weasyprint_mode_broken_when_oserror(monkeypatch: pytest.MonkeyPatch) -> None:
    """Mode reports broken when rendering raises OSError."""
...
def test_assert_weasyprint_available_passes_in_real_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    """Availability assertion does not raise when mode is real."""
    monkeypatch.setattr(export_service, "get_weasyprint_mode", lambda: "real")
    assert_weasyprint_available()


def test_assert_weasyprint_available_passes_in_stub_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    """Availability assertion does not raise in accepted stub mode."""
    monkeypatch.setattr(export_service, "get_weasyprint_mode", lambda: "stub")
    assert_weasyprint_available()


def test_assert_weasyprint_available_raises_in_broken_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    """Availability assertion raises actionable error for broken mode."""
...
def test_pdf_generation_calls_assert_weasyprint(monkeypatch: pytest.MonkeyPatch) -> None:
    """PDF export calls capability assertion exactly once before render."""
    assert_mock = Mock()
...
def test_pdf_generation_propagates_broken_error(monkeypatch: pytest.MonkeyPatch) -> None:
    """PDF export propagates capability RuntimeError without swallowing it."""
    monkeypatch.setattr(
        export_service,
        "assert_weasyprint_available",
...
def test_pdf_generation_returns_bytes_on_success(monkeypatch: pytest.MonkeyPatch) -> None:
    """PDF export returns non-empty PDF bytes on successful render."""
...
def test_pdf_generation_fails_on_empty_output(monkeypatch: pytest.MonkeyPatch) -> None:
    """PDF export raises when renderer returns empty bytes."""
...
def test_smoke_script_exits_zero_on_success(monkeypatch: pytest.MonkeyPatch) -> None:
    """Smoke script returns zero when minimal PDF render succeeds."""
...
def test_smoke_script_exits_one_on_oserror(monkeypatch: pytest.MonkeyPatch) -> None:
    """Smoke script returns one when WeasyPrint raises missing-library OSError."""

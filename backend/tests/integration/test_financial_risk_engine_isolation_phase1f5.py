...
async def test_risk_run_does_not_modify_engine_or_journal_tables(
    financial_risk_phase1f5_session: AsyncSession,
) -> None:
    tenant_id = uuid.uuid4()
    await ensure_tenant_context(financial_risk_phase1f5_session, tenant_id)
    before_engine = await _counts(financial_risk_phase1f5_session, ENGINE_TABLES)
    before_journal = await _counts(financial_risk_phase1f5_session, JOURNAL_TABLES)
    ...
    after_engine = await _counts(financial_risk_phase1f5_session, ENGINE_TABLES)
    after_journal = await _counts(financial_risk_phase1f5_session, JOURNAL_TABLES)
...
@pytest.mark.asyncio
@pytest.mark.integration
async def test_risk_run_does_not_mutate_normalization_reconciliation_ratio_inputs(
    financial_risk_phase1f5_session: AsyncSession,
) -> None:
    tenant_id = uuid.uuid4()
    await ensure_tenant_context(financial_risk_phase1f5_session, tenant_id)
    ...
    before_upstream = await _counts(financial_risk_phase1f5_session, UPSTREAM_TABLES)
    ...
    after_upstream = await _counts(financial_risk_phase1f5_session, UPSTREAM_TABLES)
    assert before_upstream == after_upstream


@pytest.mark.asyncio
@pytest.mark.integration
async def test_risk_run_does_not_invoke_fx_tables(
    financial_risk_phase1f5_session: AsyncSession,
) -> None:
    tenant_id = uuid.uuid4()
    await ensure_tenant_context(financial_risk_phase1f5_session, tenant_id)
    before_fx = await _counts(financial_risk_phase1f5_session, FX_TABLES)
    ...
    after_fx = await _counts(financial_risk_phase1f5_session, FX_TABLES)

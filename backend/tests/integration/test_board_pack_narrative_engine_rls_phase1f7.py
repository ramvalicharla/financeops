...
async def test_tenant_can_read_own_board_pack_definition(
    ...
    async with session_factory() as session:
        ...
        try:
            ...
            await session.execute(text("SET ROLE rls_board_pack_probe_user"))
            try:
                await ensure_tenant_context(session, tenant_id)
                count = (
                    await session.execute(text("SELECT COUNT(*) FROM board_pack_definitions"))
                ).scalar_one()
                assert count >= 1
            finally:
    ...
    async with session_factory() as session:
        ...
        try:
            ...
            try:
                ...
                anomaly_run_id = upstream["anomaly_run_id"]
                with pytest.raises(DBAPIError):
                    await session.execute(
                        ...
                        ),
                        {
                            ...
                            "ttoken": "c" * 64,
                            "itoken": "d" * 64,
                            "metric": f'["{metric_run_id}"]',
                            "risk": f'["{risk_run_id}"]',
                            "anomaly": f'["{anomaly_run_id}"]',

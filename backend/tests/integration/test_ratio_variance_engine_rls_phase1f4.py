...
async def test_tenant_can_read_own_metric_definition(
    ...
    await _configure_probe_role(ratio_phase1f4_session)
    ...
    await ratio_phase1f4_session.execute(text("SET ROLE rls_ratio_probe_user"))
    try:
        await ensure_tenant_context(ratio_phase1f4_session, tenant_id)
        count = (
            await ratio_phase1f4_session.execute(text("SELECT COUNT(*) FROM metric_definitions"))
        ).scalar_one()
        assert count >= 1
    finally:
        if ratio_phase1f4_session.in_transaction():
            await ratio_phase1f4_session.rollback()
        await ratio_phase1f4_session.execute(text("RESET ROLE"))
...
async def test_tenant_cannot_read_other_tenant_metric_definition(
    ...
    tenant_b = uuid.uuid4()
    await _configure_probe_role(ratio_phase1f4_session)
    await ensure_tenant_context(ratio_phase1f4_session, tenant_b)
    await seed_active_definition_set(
        ratio_phase1f4_session,
        tenant_id=tenant_b,
    ...
    await ratio_phase1f4_session.execute(text("SET ROLE rls_ratio_probe_user"))
    try:
        await ensure_tenant_context(ratio_phase1f4_session, tenant_a)
        count = (
            await ratio_phase1f4_session.execute(text("SELECT COUNT(*) FROM metric_definitions"))
        ).scalar_one()
        assert count == 0

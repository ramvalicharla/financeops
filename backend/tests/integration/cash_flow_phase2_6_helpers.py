...
async def cash_flow_phase2_6_db_url() -> AsyncGenerator[str, None]:
    ...
    if migration.returncode != 0:
        raise RuntimeError(
            "alembic upgrade head failed for cash flow phase2.6 temp database\n"
            f"stdout:\n{migration.stdout}\n"
            f"stderr:\n{migration.stderr}"
        )
...
async def seed_control_plane_for_cash_flow(
    ...
    *,
    ...
    module = (
        await session.execute(
            select(CpModuleRegistry).where(CpModuleRegistry.module_code == "cash_flow_engine")
        )
    ).scalar_one_or_none()
    if module is None:
        module = CpModuleRegistry(
            module_code="cash_flow_engine",
            module_name="Cash Flow Engine",
    ...
    role = await create_role(
        session,
        tenant_id=tenant_id,
        role_code=f"CASHFLOW_ROLE_{uuid.uuid4().hex[:8]}",
        role_scope="tenant",
    ...
    permissions = [
        ("cash_flow_statement_definition", "cash_flow_definition_manage"),
        ("cash_flow_line_mapping", "cash_flow_mapping_manage"),
        ("cash_flow_bridge_rule_definition", "cash_flow_bridge_rule_manage"),
        ("cash_flow_run", "cash_flow_run"),
        ("cash_flow_run", "cash_flow_view"),
        ("cash_flow_evidence_link", "cash_flow_evidence_view"),
        ("cash_flow_statement_definition", "cash_flow_view"),
        ("cash_flow_line_mapping", "cash_flow_view"),
        ("cash_flow_bridge_rule_definition", "cash_flow_view"),
    ]
    for resource_type, action in permissions:
        ...
        if permission is None:
            permission = await create_permission(
                ...
                action=action,
                description="cash flow phase2.6 integration permission",
            )

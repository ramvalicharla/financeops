from __future__ import annotations

import uuid
from datetime import date

import pytest
from sqlalchemy import text
from sqlalchemy.ext.asyncio import AsyncSession

from tests.integration.anomaly_pattern_phase1f6_helpers import (
    build_anomaly_service,
    ensure_tenant_context,
    seed_active_anomaly_configuration,
    seed_upstream_for_anomaly,
)

ENGINE_TABLES = (
    "revenue_schedules",
    "lease_liability_schedule",
    "prepaid_amortization_schedule",
    "asset_depreciation_schedule",
)

JOURNAL_TABLES = (
    "revenue_journal_entries",
    "lease_journal_entries",
    "prepaid_journal_entries",
    "asset_journal_entries",
)

UPSTREAM_TABLES = (
    "metric_runs",
    "metric_results",
    "variance_results",
    "trend_results",
    "risk_runs",
    "risk_results",
    "reconciliation_sessions",
    "reconciliation_lines",
    "reconciliation_exceptions",
    "normalization_runs",
    "payroll_normalized_lines",
    "gl_normalized_lines",
)

FX_TABLES = (
    "fx_rate_fetch_runs",
    "fx_rate_quotes",
    "fx_manual_monthly_rates",
    "fx_variance_results",
)


async def _counts(session: AsyncSession, tables: tuple[str, ...]) -> dict[str, int]:
    data: dict[str, int] = {}
    for table in tables:
        count = (await session.execute(text(f"SELECT COUNT(*) FROM {table}"))).scalar_one()
        data[table] = int(count)
    return data


async def _run_anomaly_flow(session: AsyncSession, *, tenant_id: uuid.UUID) -> None:
    await ensure_tenant_context(session, tenant_id)
    upstream = await seed_upstream_for_anomaly(
        session,
        tenant_id=tenant_id,
        organisation_id=tenant_id,
        created_by=tenant_id,
        reporting_period=date(2026, 1, 31),
    )
    await seed_active_anomaly_configuration(
        session,
        tenant_id=tenant_id,
        organisation_id=tenant_id,
        created_by=tenant_id,
        effective_from=date(2026, 1, 1),
    )
    service = build_anomaly_service(session)
    created = await service.create_run(
        tenant_id=tenant_id,
        organisation_id=tenant_id,
        reporting_period=date(2026, 1, 31),
        source_metric_run_ids=[uuid.UUID(upstream["metric_run_id"])],
        source_variance_run_ids=[uuid.UUID(upstream["metric_run_id"])],
        source_trend_run_ids=[uuid.UUID(upstream["metric_run_id"])],
        source_risk_run_ids=[uuid.UUID(upstream["risk_run_id"])],
        source_reconciliation_session_ids=[],
        created_by=tenant_id,
    )
    await service.execute_run(
        tenant_id=tenant_id,
        run_id=uuid.UUID(created["run_id"]),
        actor_user_id=tenant_id,
    )


@pytest.mark.asyncio
@pytest.mark.integration
async def test_anomaly_run_does_not_modify_engine_or_journal_tables(
    anomaly_phase1f6_session: AsyncSession,
) -> None:
    tenant_id = uuid.uuid4()
    await ensure_tenant_context(anomaly_phase1f6_session, tenant_id)
    before_engine = await _counts(anomaly_phase1f6_session, ENGINE_TABLES)
    before_journal = await _counts(anomaly_phase1f6_session, JOURNAL_TABLES)
    await _run_anomaly_flow(anomaly_phase1f6_session, tenant_id=tenant_id)
    after_engine = await _counts(anomaly_phase1f6_session, ENGINE_TABLES)
    after_journal = await _counts(anomaly_phase1f6_session, JOURNAL_TABLES)
    assert before_engine == after_engine
    assert before_journal == after_journal


@pytest.mark.asyncio
@pytest.mark.integration
async def test_anomaly_run_does_not_mutate_upstream_ratio_risk_reconciliation_or_normalization(
    anomaly_phase1f6_session: AsyncSession,
) -> None:
    tenant_id = uuid.uuid4()
    await ensure_tenant_context(anomaly_phase1f6_session, tenant_id)
    upstream = await seed_upstream_for_anomaly(
        anomaly_phase1f6_session,
        tenant_id=tenant_id,
        organisation_id=tenant_id,
        created_by=tenant_id,
        reporting_period=date(2026, 1, 31),
    )
    await seed_active_anomaly_configuration(
        anomaly_phase1f6_session,
        tenant_id=tenant_id,
        organisation_id=tenant_id,
        created_by=tenant_id,
        effective_from=date(2026, 1, 1),
    )
    before_upstream = await _counts(anomaly_phase1f6_session, UPSTREAM_TABLES)
    service = build_anomaly_service(anomaly_phase1f6_session)
    created = await service.create_run(
        tenant_id=tenant_id,
        organisation_id=tenant_id,
        reporting_period=date(2026, 1, 31),
        source_metric_run_ids=[uuid.UUID(upstream["metric_run_id"])],
        source_variance_run_ids=[uuid.UUID(upstream["metric_run_id"])],
        source_trend_run_ids=[uuid.UUID(upstream["metric_run_id"])],
        source_risk_run_ids=[uuid.UUID(upstream["risk_run_id"])],
        source_reconciliation_session_ids=[],
        created_by=tenant_id,
    )
    await service.execute_run(
        tenant_id=tenant_id,
        run_id=uuid.UUID(created["run_id"]),
        actor_user_id=tenant_id,
    )
    after_upstream = await _counts(anomaly_phase1f6_session, UPSTREAM_TABLES)
    assert before_upstream == after_upstream


@pytest.mark.asyncio
@pytest.mark.integration
async def test_anomaly_run_does_not_invoke_fx_tables(
    anomaly_phase1f6_session: AsyncSession,
) -> None:
    tenant_id = uuid.uuid4()
    await ensure_tenant_context(anomaly_phase1f6_session, tenant_id)
    before_fx = await _counts(anomaly_phase1f6_session, FX_TABLES)
    await _run_anomaly_flow(anomaly_phase1f6_session, tenant_id=tenant_id)
    after_fx = await _counts(anomaly_phase1f6_session, FX_TABLES)
    assert before_fx == after_fx

...
async def equity_phase2_7_db_url() -> AsyncGenerator[str, None]:
    ...
    if migration.returncode != 0:
        raise RuntimeError(
            "alembic upgrade head failed for equity phase2.7 temp database\n"
            f"stdout:\n{migration.stdout}\n"
            f"stderr:\n{migration.stderr}"
        )
...
async def seed_control_plane_for_equity(
    ...
    *,
    ...
    module = (
        await session.execute(
            select(CpModuleRegistry).where(CpModuleRegistry.module_code == "equity_engine")
        )
    ).scalar_one_or_none()
    if module is None:
        module = CpModuleRegistry(
            module_code="equity_engine",
            module_name="Equity Engine",
    ...
    role = await create_role(
        session,
        tenant_id=tenant_id,
        role_code=f"EQUITY_ROLE_{uuid.uuid4().hex[:8]}",
        role_scope="tenant",
    ...
    permissions = [
        ("equity_statement_definition", "equity_definition_manage"),
        ("equity_line_definition", "equity_definition_manage"),
        ("equity_source_mapping", "equity_definition_manage"),
        ("equity_rollforward_rule_definition", "equity_rule_manage"),
        ("equity_run", "equity_run"),
        ("equity_run", "equity_view"),
        ("equity_statement_definition", "equity_view"),
        ("equity_rollforward_rule_definition", "equity_view"),
    ]
    for resource_type, action in permissions:
        ...
        if permission is None:
            permission = await create_permission(
                ...
                action=action,
                description="equity phase2.7 integration permission",
            )

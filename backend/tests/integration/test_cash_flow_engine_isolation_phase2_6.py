    ...
    try:
        async with session_factory() as session:
            ...
            try:
                ...
                revenue_before = await session.scalar(select(func.count()).select_from(RevenueJournalEntry))
                lease_before = await session.scalar(select(func.count()).select_from(LeaseJournalEntry))
                prepaid_before = await session.scalar(select(func.count()).select_from(PrepaidJournalEntry))
                ...
                revenue_after = await session.scalar(select(func.count()).select_from(RevenueJournalEntry))
                lease_after = await session.scalar(select(func.count()).select_from(LeaseJournalEntry))
                prepaid_after = await session.scalar(select(func.count()).select_from(PrepaidJournalEntry))

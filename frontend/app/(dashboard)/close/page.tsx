...
export default function ClosePage() {
  ...
  const lockMutation = useMutation({
    mutationFn: (lockType: "SOFT_CLOSED" | "HARD_CLOSED") => {
      const reason = window.prompt(`Reason for ${lockType.replace("_", " ").toLowerCase()}?`)?.trim() ?? ""
      return lockPeriod({
        org_entity_id: activeEntityId ?? undefined,
        fiscal_year: fiscalYear,
        period_number: periodNumber,
  ...
  const unlockMutation = useMutation({
    mutationFn: () => {
      const reason = window.prompt("Unlock reason (required)")?.trim() ?? ""
      if (!reason) {
            ...
            <div className="flex flex-wrap gap-2">
              {roleCanLock(userRole) ? (
                <>
                  <Button
                    variant="outline"
                    onClick={() => lockMutation.mutate("SOFT_CLOSED")}
                  ...
                  <Button
                    variant="outline"
                    onClick={() => lockMutation.mutate("HARD_CLOSED")}
                    disabled={lockMutation.isPending || !activeEntityId}
                  >
                    Hard Close
                  </Button>

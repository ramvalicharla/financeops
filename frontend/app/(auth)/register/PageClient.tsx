    ...
    <div className="rounded-lg border border-border bg-card p-6 shadow-sm">
      ...
      <div className="space-y-4">
        <FormField id="register-full-name" error={errors.fullName} label="Full Name" required>
          <Input autoComplete="name" placeholder="Full Name" value={form.fullName} onChange={(e) => setForm((f) => ({ ...f, fullName: e.target.value }))} />
        </FormField>
        <FormField id="register-email" error={errors.email} label="Work Email" required>
          <Input autoComplete="email" type="email" placeholder="Work Email" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} />
        </FormField>
        <FormField id="register-company-name" error={errors.companyName} label="Company / Firm Name" required>
          <Input autoComplete="organization" placeholder="Company / Firm Name" value={form.companyName} onChange={(e) => setForm((f) => ({ ...f, companyName: e.target.value }))} />
        </FormField>
        ...
        <FormField id="register-password" error={errors.password} label="Password" required>
          <Input autoComplete="new-password" type="password" placeholder="Password (min 8 chars)" value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))} />
        </FormField>
        <FormField id="register-confirm-password" error={errors.confirmPassword} label="Confirm Password" required>
          <Input autoComplete="new-password" type="password" placeholder="Confirm Password" value={form.confirmPassword} onChange={(e) => setForm((f) => ({ ...f, confirmPassword: e.target.value }))} />
        </FormField>
